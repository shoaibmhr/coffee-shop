...
import { X, ImagePlus, Link2, Upload } from "lucide-react";
...
// Same pattern as ProductFormModal: state is initialized once from props,
// and the parent passes a `key` (category?.id || "new") so React remounts
// this component fresh instead of needing a useEffect to sync state.
const CategoryFormModal = ({ category, onClose, onSave }) => {
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      // Backend connect hone par: file ko FormData me daal kar
      // POST /api/upload jaisi API call hogi, response se real URL milega
    }
  };
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4">
            ...
            {/* Status toggle — same fixed pattern as ProductFormModal */}
            <div className="flex items-center justify-between p-3.5 rounded-lg bg-coffee-cream/50">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full shrink-0 transition-colors ${
                  formData.status === "Active" ? "bg-coffee-accent" : "bg-coffee-dark/20"
                }`}
              >
                <motion.span
